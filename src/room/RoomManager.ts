...
export default {
	...
	async create({ name, hostUserId, capacity = 5 } : { name: string, hostUserId: string, participants: string[], capacity: number }) {
		const newRoom = {
			name, 
			hostUserId, 
			capacity,
	...
	async changeHost(roomId: string, newHost: string) {
		...
		if (room.hostUserId === user._id.toString()) throw Boom.badData(errors.room.USER_ALREADY_HOST);
	...
	async joinOrLeaveRoom({ action, roomId, userId } : { action: string, roomId: string, userId: string }) {
		const [room, user] = await Promise.all([
			this.get(roomId), 
			UserManager.get(userId)
		]);

		await (action === 'join' ? this.join({ room, user }) : this.leave({ room, user }));
	...
	join({ room, user }: { room: IRoom, user: IUser }) {
		...
		if (room.participants!.length === room.capacity) throw Boom.badData(errors.room.MAXIMUM_CAPACITY_REACHED);
		if (room.hostUserId === userId || room.participantsIds.includes(userId)) throw Boom.forbidden(errors.room.JOIN_USER_ALREADY_IN_ROOM)

		return Room.findByIdAndUpdate(room._id, { $push: { participantsIds: user._id }})
	},

	leave({ room, user }: { room: IRoom, user: IUser}) {

		if (room.hostUserId === user._id.toString()) throw Boom.badData(errors.room.HOST_UNABLE_LEAVE_ROOM);
		if (!room.participantsIds.includes(user._id.toString())) throw Boom.badData(errors.room.USER_NOT_IN_ROOM);

		return Room.findByIdAndUpdate(room._id, { $pull: { participantsIds: user._id }})
	}
}
