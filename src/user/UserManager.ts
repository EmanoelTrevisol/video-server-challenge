...
export default {
	...
	async get(userId: string, selectPassword = false) {
		const user = await User.findById(userId).select(selectPassword ? '+password' : '');
	...
	async register({ username, password, mobile_token } : { username: string, password: string, mobile_token: string }) {
		const newUser = {
			username, 
	...
	async signIn({ username, password }: { username: string, password: string}) {
		...
		if (!user || !(await checkPassword(password, user.password!))) throw Boom.unauthorized(errors.user.WRONG_CREDENTIALS)
	...
	async update({ userId, password, mobile_token: mobileToken } : { userId: string, password: string, mobile_token: string}) {
